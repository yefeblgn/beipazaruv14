...
module.exports = {
    ...
    execute: async (client, message, args, embed) => {
        ...
        if (member && member.presence && member.presence.activities && member.presence.activities.some(five => five.name == "Spotify" && five.type == ActivityType.Listening)) {
          ...
          return message.reply({files:[{name:"canvafy.png",attachment:spotify}],embeds: [embed.setDescription(
            `**
            ...
            `).setThumbnail(member.user.displayAvatarURL({dynamic:true})).setTitle(`${member.user.tag} Bilgileri`)] });
        }
