...
module.exports = {
    ...
    execute: async (client, message, args, embed) => {
        ...
        if(member.id == message.author.id) return message.reply({ embeds: [embed.setDescription(`> **Kendine İşlem Uygulayamazsın!**`)]}).sil(5);
        if(member.user.bot) return message.reply({ embeds: [embed.setDescription(`> **Bir Bot'a İşlem Uygulayamazsın!**`)]}).sil(5);
        if(member.roles.highest.position >= message.member.roles.highest.position) return message.reply({ embeds: [embed.setDescription(`> **Kendinden Üst/Aynı Pozisyondaki Birine İşlem Uygulayamazsın!**`)]}).sil(5);
        if (limitData >= config.limit.muteLimit)return message.reply({ embeds: [embed.setDescription(`> **Mute Atma Limitin Dolmuş Durumda, Lütfen Sonra Tekrar Dene!**`)]}).sil(5);
        if (!time || !["w","d","h","m","s"].some(bes => time.includes(bes)))return message.reply({ embeds: [embed.setDescription(`> **Geçerli Bir Süre Belirt!**`)]}).sil(5);
        if (reason.length < 1)return message.reply({ embeds: [embed.setDescription(`> **Geçerli Bir Sebep Belirt!**`)]}).sil(5);
        if(!member.manageable)return message.reply({ embeds: [embed.setDescription(`> **Belirtilen User'a İşlem Yapmaya Yetkim Yetmiyor!**`)]}).sil(5);
        ...
        let mesaj = await message.reply({ components:[buttons],embeds: [embed.setDescription(`> **${client.emoji("emote_value") !== null ? client.emoji("emote_value") : "❓"} Aşağıdaki Butonlardan Bir İşlem Tipi Belirleyiniz!**`)]})
