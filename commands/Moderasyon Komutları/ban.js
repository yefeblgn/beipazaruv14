...
module.exports = {
    ...
    execute: async (client, message, args, embed) => {
        ...
        if (limitData >= config.limit.banLimit)return message.reply({ embeds: [embed.setDescription(`> **Banlama Limitin Dolmuş Durumda, Lütfen Sonra Tekrar Dene!**`)]}).sil(5);
        if (reason.length < 1)return message.reply({ embeds: [embed.setDescription(`> **Geçerli Bir Sebep Belirt!**`)]}).sil(5);
        if(!member.bannable)return message.reply({ embeds: [embed.setDescription(`> **Belirtilen User'a İşlem Yapmaya Yetkim Yetmiyor!**`)]}).sil(5);
