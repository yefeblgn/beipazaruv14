...
module.exports = {
    ...
    execute: async (client, message, args, embed) => {
        ...
        if(member.bot) return message.reply({ embeds: [embed.setDescription(`> **Bir Bot'a İşlem Uygulayamazsın!**`)]}).sil(5);
        if (reason.length < 1)return message.reply({ embeds: [embed.setDescription(`> **Geçerli Bir Sebep Belirt!**`)]}).sil(5);
        if(forcedata.includes(member.id)) return message.reply({ embeds: [embed.setDescription(`> **Belirtilen Üyenin Zaten Bir Kalıcı Yasaklaması Bulunmakta!**`)]}).sil(5);
