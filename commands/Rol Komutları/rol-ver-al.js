...
module.exports = {
    ...
    execute: async (client, message, args, embed) => {
        ...
        if (user.user.bot) return message.reply({ embeds: [embed.setDescription(`> **Bir Bot'a İşlem Uygulayamazsın!**`)]}).sil(5);
        let role = message.mentions.roles.first() || message.guild.roles.cache.get(args[2]);
        if (!role) return message.reply({ embeds: [embed.setDescription(`> **Geçerli Bir Rol Belirt!**`)] }).sil(5);
        ...
        if (!role.editable) return message.reply({ embeds: [embed.setDescription(`> **Belirtilen Role İşlem Yapmaya Yetkim Yetmiyor!**`)] }).sil(5);
        if (args[0] == "al") {
            if (user.roles.cache.has(role.id)) {
                ...
                if (client.kanalbul("rol-log")) client.kanalbul("rol-log").send({
                    embeds: [embed.setDescription(`> **${user} Kullanıcısından ${role} Rolü Alındı!**`).addFields(
                ...
                return message.reply({ embeds: [embed.setDescription(`> **${user} Kullanıcısında ${role} Rolü Mevcut Değil!**`)] }).sil(5);
            }
        }
        if (args[0] == "ver") {
            if (!user.roles.cache.has(role.id)) {
                ...
                db.push(`rollog-${user.id}`,`\`[+]\` ${message.author} (\`${message.author.id}\`)\n*Tarih;* **<t:${Math.floor(Date.now()/1000)}> (<t:${Math.floor(Date.now()/1000)}:R>)**\n*Rol;* ${role} (\`${role.id}\`)`)
                if (client.kanalbul("rol-log")) client.kanalbul("rol-log").send({
                    embeds: [embed.setDescription(`> **${user} Kullanıcısına ${role} Rolü Verildi!**`).addFields(
                ...
                client.false(message)
                return message.reply({ embeds: [embed.setDescription(`> **${user} Kullanıcısında ${role} Rolü Zaten Mevcut!**`)] }).sil(5);
            }
        }
    }
}
