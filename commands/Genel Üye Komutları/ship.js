...
module.exports = {
    ...
    execute: async (client, message, args, embed) => {
        if(!["ship","bot","commands","command","komut"].some(bes => message.channel.name.includes(bes))) return message.channel.send({ embeds: [embed.setDescription(`> **Ship Komudunu Sadece Adında "ship","bot","commands","command","komut" İçeren Kanallarda Kullanabilirsin!**`)] }).sil(5);
        let user = message.mentions.members.first() || message.guild.members.cache.get(args[0]) || message.guild.members.cache.random();
        if(!user) return message.channel.send({ embeds: [embed.setDescription(`> **Geçerli Bir User Belirt!**`)] }).sil(5);
