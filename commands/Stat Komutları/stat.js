...
module.exports = {
    ...
    embed.setDescription(`> **${member.user.toString()} Kullanıcısının <t:${Math.floor(Date.now()/1000)}:R>ki Verileri;**\n⠀\n`)
    embed.addFields([{name: `${client.emoji("emote_chat") !== null ? client.emoji("emote_chat"):"💭"} • **Toplam Mesaj**`, value: codeBlock("fix",`${messageData ? messageData.topStat : 0} Mesaj`)},
    ...
    embed.addFields([{name:`${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} • **Sohbet Ettiğin Chat Kanalları:**\n> **(\`Toplam ${chatLength} Kanal\`)**`,value: `${messageTop}`}]);
    embed.addFields([{name:`${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} • **Vakit Geçirdiğin Voice Kanalları:**\n> **(\`Toplam ${voiceLength} Kanal\`)**`,value:`${voiceTop}`}]);
    embed.setFooter({text:`Menü Üzerinden Verilerinize Filtre Uygulayabilirsiniz`,iconURL:member.user.avatarURL({ dynamic: true, size: 2048 })})

    message.reply({ embeds: [embed], components: [dropdown] }).then(b2 => {
            ...
            b2.edit({ content: `> **Menü Zaman Aşımına Uğradı..**`, components: [dropdown] })
            })
        ...
        **${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} • Public & Sohbet: \`${await category(config.parents.publicParents)}\`**
        **${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} • Kayıt & Welcome: \`${await category(config.parents.registerParents)}\`**
        **${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} • Sorun Çözme & Terapi: \`${await category(config.parents.solvingParents)}\`**
        **${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} • Private & Alone: \`${await category(config.parents.privateParents)}\`**
        **${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} • Oyun & Stream: \`${await category(config.parents.aloneParents)}\`**
        **${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} • Konser & Eğlence: \`${await category(config.parents.funParents)}\`**`}]
