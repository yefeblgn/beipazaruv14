...
module.exports = {
    ...
    category:"sahip",
    usage:"eval",
    aliases: [], execute: async (client, message, args, embed) => {
        if (!config.botOwners.some(bes => message.author.id == bes)) return;
        if (!args[0]) return message.reply({ content: `> **Kod Nerede Canımın İçi!**` });
        let code = args.join(" ");
        if (code.includes(client.token)) return message.reply({ content: "> **Bu Token Koruma Altında ;)**" });
        try {
            var sonuç = eval_beş(await eval(code));
            if (sonuç.includes(client.token))
                return message.reply({ content: "> **Bu Token Koruma Altında ;)**" });
        } catch (err) { }
    },
