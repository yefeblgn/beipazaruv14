...
module.exports = {
    ...
    execute: async (client, message, args, embed) => {
        ...
        const canvasPanel = await new canvafy.Rank()
            .setAvatar(`${message.guild.iconURL({ forceStatic: true, extension: "png" }) !== null ? message.guild.iconURL({ forceStatic: true, extension: "png" }) : config.shipArkaplan}`)
...
`).setThumbnail(message.guild.iconURL({ forceStatic: true }))
        ...
        if (!secim) {
            ...
            collector.on('collect', async (beş) => {
                ...
                if (beş.customId == "regsetup") {
                    beş.update({ content: ``, embeds: [registerEmbed], components: [buttons],files:[] })
                    collector.stop();
                } else if (beş.customId == "modsetup") {
                    beş.update({ content: ``, embeds: [moderationEmbed], components: [buttons],files:[] })
                    collector.stop();
        ...
        if (secim == "1") {
            ...
            if (!roles) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${config.prefix}setup 3 @rol**` })
            ...
            if (!roles) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${config.prefix}setup 4 @rol**` })
            ...
            if (!roles) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${config.prefix}setup 5 @rol**` })
            ...
            if (!channel) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${config.prefix}setup 9 #chat-kanal**` })
            ...
            if (!roles) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${config.prefix}setup 10 @rol**` })
            ...
            if (!roles) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${config.prefix}setup 11 @rol**` })
            ...
            if (!roles) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${config.prefix}setup 12 @rol**` })
            ...
            if (!roles) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${config.prefix}setup 13 @rol**` })
            ...
            if (!roles) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${config.prefix}setup 14 @rol**` })
            ...
            if (!roles) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${config.prefix}setup 15 @rol**` })
            ...
            if (!roles) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${config.prefix}setup 16 @rol**` })
            if (cmutestaffData.some(bes => bes.includes(roles.id))) {
                ...
                if (!roles) return message.reply({ content: `> **❌ Hatalı Kullanım!**\n> **\`Örnek;\` ${config.prefix}setup 17 @rol**` })
...
client.on(Events.InteractionCreate,async(beş) => {
 if(!beş.isButton())return;
  ...
  if(value == "burcrolkur"){
    ...
    const burc = config.etkinlikRoles;
    for (let index = 0; index < burc.length; index++) {
        let element = burc[index];
        if(beş.guild.premiumTier >= 2){
        await beş.guild.roles.create({
