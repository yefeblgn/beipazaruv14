...
module.exports = {
    ...
    const msgMember = message.member;
           ...
           if(!args[0])return message.reply({ embeds: [embed.setDescription("> **Eksik Argüman Belirtmelisin!**\n> *ekle / sil / gönder*")] });
            ...
            if (args[0] === "ekle") {
                ...
                if (!count) return message.reply({ embeds: [embed.setDescription("> **Eklemek İçin Bir Sayı Belirtmelisin!**")] });
                if (!count < 0) return message.reply({ embeds: [embed.setDescription("> **Eklenecek Sayı 0'dan Küçük Olamaz!**")] });
                ...
                message.reply({
                 ...
                 if (!message.member.permissions.has("ADMINISTRATOR")) return message.reply({ embeds: [embed.setDescription(`> **${config.emojis.no} Komutu Kullanmak İçin Yetkin Bulunmamakta!**`)] }).catch((err) => console.log(err), client.tick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
                const count = parseInt(args[2]);
                if (!count) return message.reply({ embeds: [embed.setDescription("> **Çıkarılacak Bir Sayı Belirtmelisin!**")] });
                if (!count < 0) return message.reply({ embeds: [embed.setDescription("> **Çıkarılacak Sayı 0'dan Küçük Olamaz!**")] });
                let pointData = await point.findOne({ guildId: member.guild.id, userId: member.user.id });
                if (!pointData || (pointData && count > pointData.point)) return message.reply({ embeds: [embed.setDescription("> **Çıkarmak İstediğiniz Sayı Kişinin Mevcut Puanından Büyük Olamaz!**")] });
                ...
                const count = parseInt(args[2]);
                if (!count) return message.reply({ embeds: [embed.setDescription("> **Puan Vermek İçin Bir Sayı Belirtmelisin!**")] });
                if (!count < 0) return message.reply({ embeds: [embed.setDescription("> **Verilecek Sayı 0'dan Küçük Olamaz!**")] });
                let pointData = await point.findOne({ guildId: member.guild.id, userId: msgMember.user.id });
                if (!pointData || (pointData && count > pointData.point)) return message.reply({ embeds: [embed.setDescription("> **Göndereceğin Puan Kendi Puanından Yüksek Olamaz!**")] });
                ...
                message.reply({ embeds: [embed.setDescription(`> **${member.toString()} Kişisine ${count} Puan Gönderildi!**`)] });
            }else{
    return message.reply({ embeds: [embed.setDescription("> **Eksik Argüman Belirtmelisin!**\n> *ekle / sil / gönder*")] });
