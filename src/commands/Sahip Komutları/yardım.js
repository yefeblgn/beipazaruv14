...
switch (value) {
    case "genelkomutlar":
        ...
        beş.reply({ephemeral:true,content:codeBlock("fix",`${cmd.map(bes => `${config.prefix}${bes.usage}`).join("\n")}`)})
        break;
        case "kayıtkomutlar":
        let cmd2 = commandShow("kayıt");
        beş.reply({ephemeral:true,content:codeBlock("fix",`${cmd2.map(bes => `${config.prefix}${bes.usage}`).join("\n")}`)})
        break;
        case "moderasyonkomutlar":
        let cmd3 = commandShow("moderasyon");
        beş.reply({ephemeral:true,content:codeBlock("fix",`${cmd3.map(bes => `${config.prefix}${bes.usage}`).join("\n")}`)})
        break;
        case "rolkomutlar":
        let cmd4 = commandShow("rol");
        beş.reply({ephemeral:true,content:codeBlock("fix",`${cmd4.map(bes => `${config.prefix}${bes.usage}`).join("\n")}`)})
        break;
        case "sahipkomutlar":
        if (!config.botOwners.some(bes => beş.user.id == bes)) return beş.reply({content:`> **Bot Sahibi Değilsin!**`,ephemeral:true});
        let cmd5 = commandShow("sahip");
        beş.reply({ephemeral:true,content:codeBlock("fix",`${cmd5.map(bes => `${config.prefix}${bes.usage}`).join("\n")}`)})
        break;
        case "üstytkomutlar":
        let cmd6 = commandShow("üstyt");
        beş.reply({ephemeral:true,content:codeBlock("fix",`${cmd6.map(bes => `${config.prefix}${bes.usage}`).join("\n")}`)})
        break;
        case "statkomutlar":
        let cmd7 = commandShow("stat");
        beş.reply({ephemeral:true,content:codeBlock("fix",`${cmd7.map(bes => `${config.prefix}${bes.usage}`).join("\n")}`)})
        break;
}
}
})
