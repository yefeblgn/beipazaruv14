...
module.exports = {
    ...
    execute: async (client, message, args, embed) => {
        ...
        if (!user.voice.channel) return message.reply({ embeds: [embed.setDescription(`> **Geçerli Ses Kanalında Bulunan Bir User Belirt!**`)] }).sil(5);
        if(user.roles.highest.position >= message.member.roles.highest.position) return message.reply({ embeds: [embed.setDescription(`> **Kendinden Üst/Aynı Pozisyondaki Birine İşlem Uygulayamazsın!**`)]}).sil(5);
