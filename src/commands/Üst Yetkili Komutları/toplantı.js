...
module.exports = {
    ...
    execute: async (client, message, args, embed) => {
        ...
        if (!katıldı) return message.reply({ embeds: [embed.setDescription(`> **Sunucuda \`katıldı\` Adında Rol Bulunamadı!**`)] }).sil(5);
        let uyarı = client.rolinc("uyarı");
        if (!uyarı) return message.reply({ embeds: [embed.setDescription(`> **Sunucuda \`uyarı\` Adında Rol Bulunamadı!**`)] }).sil(5);
        let mazeretli = client.rolinc("mazeret");
        if (!mazeretli) return message.reply({ embeds: [embed.setDescription(`> **Sunucuda \`mazeretli\` Adında Rol Bulunamadı!**`)] }).sil(5);
        let altytrol = await db.get("five-register-staff") || [];
        if (!altytrol.length > 0) return message.reply({ embeds: [embed.setDescription(`> **Kayıt Yetkisi Olmadığı İçin İşlem Gerçekleştirilemedi!** *Lütfen Setup Panelden Kayıt Yetkisini Ayarlayınız.*`)] }).sil(5);
        ...
        let mesaj = await message.reply({ components: [buttons], embeds: [embed.setDescription(`> **Menuden Bir \`Toplantı\` İşlemi Belirtiniz!**`)] })
        ...
        collector.on('collect', async (beş) => {
            ...
            break;
                case "meetingcall":
                 ...
                 if(nnvoiceuser.length == 0)return beş.reply({ embeds: [embed.setDescription(`> **Sunucudaki Tüm Yetkililer Seste Bulunuyor!**`)] }).sil(15);
                 let mesaj = await beş.reply({ embeds: [embed.setDescription(`> **Seste Olmayan ${client.sayıEmoji(nnvoiceuser.size)} Kişiye DM Üzerinden Duyuru Geçiliyor!** *Lütfen Biraz Bekleyiniz.*`)] });
                 beş.message.delete();
