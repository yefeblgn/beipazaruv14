...
module.exports = {
    ...
    execute: async (client, message, args, embed) => {
        ...
        if(forcedata.includes(member.id)) return message.reply({ embeds: [embed.setDescription(`> **Belirtilen Üyenin Bir Kalıcı Yasaklaması Bulunmakta!** *Yönetim Ekibiyle İletişime Geçin.*`)]}).sil(5);

        const fetchBans =  message.guild.bans.fetch()
        fetchBans.then(async (bans) => {
        let ban = await bans.find(a => a.user.id === member.id)
        if (!ban) {
        return message.reply({ embeds: [embed.setDescription(`> **Belirtilen Üyenin Bir Yasaklanması Bulunmamakta!**`)]}).sil(5);
        }else{ 
