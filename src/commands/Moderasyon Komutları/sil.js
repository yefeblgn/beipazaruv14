...
module.exports = {
    ...
    execute: async (client, message, args, embed) => {
        ...
        if (!args[0]) return message.reply({ components: [buttons], embeds: [embed.setDescription(`> **Silinecek Mesajın Adet Tipini Seçiniz!**`).setThumbnail(message.guild.iconURL({ forceStatic: true, dynamic: true }))] })
        if ((args[0] && isNaN(args[0])) || Number(args[0]) < 1 || Number(args[0]) > 100) return message.reply({ embeds: [embed.setDescription(`> **Eksik Komut Kullandınız!** *${config.prefix}sil <0-100>*`)] }).sil(5);
