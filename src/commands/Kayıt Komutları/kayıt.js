...
return message.reply({ embeds: [embed.setDescription(`> **Taglı Alım Açık Olduğu İçin Sadece Taglı Kullanıcılar Kayıt Edilebilir!**`)] }).sil(5)
}}
...
if (member.roles.cache.get(manRoles[0]) || member.roles.cache.get(womanRoles[0])) return message.reply({ embeds: [embed.setDescription(`> **Kayıtlı Bir Kullanıcıyı Tekrar Kayıt Edemezsin!**`)]}).sil(5);
if (member.roles.highest.position >= message.member.roles.highest.position) return message.reply({ embeds: [embed.setDescription(`> **İşlem Geçersiz Senden Üst/Aynı Pozisyonda Birisini Kayıt Edemezsin!**`)] }).sil(5);
const names = db.get(`isimler_${member.id}`)
let mesajbeş;
if(names){
mesajbeş = message.reply({ embeds: [embed.setDescription(`> **Kullanıcının ismi** \`${config.tagSymbol} ${Name2}` : ""}\` **Olarak Değiştirilecek!**\n> **Butonlardan Kullanıcının Cinsiyetini Seçiniz.**\n\n> **Kullanıcının Toplamda " ${names.length} " İsim Kayıtı Mevcut.**\n${names.slice(0,10).map((data, n) => `${data}`).join("\n")}`)], components: [dropdown] })
} 
if(!names){
mesajbeş = message.reply({ embeds: [embed.setDescription(`> **Kullanıcının ismi** \`${config.tagSymbol} ${Name2}` : ""}\` **Olarak Değiştirilecek!**\n> **Butonlardan Kullanıcının Cinsiyetini Seçiniz.**`)], components: [dropdown] })}
...
db.push(`kayıtlar-${message.author.id}`, `\`${config.tagSymbol} ${Name2}` : ""}\` (${manRoles.map((bes) => `<@&${bes}>`).join(",")} <t:${Math.floor(Date.now() / 1000)}>)`);
...
await member.setNickname(`${config.tagSymbol} ${Name2}${age ? ` ${config.symbolBeş} ${age}` : ""}`).catch(e => { })
...
message.reply({ embeds: [embed.setDescription(`> **<@${member.id}> Kullanıcının ismi** \`${config.tagSymbol} ${Name2}` : ""}\` **Olarak Değiştirildi**\n> **Ve ${config.manemoji} ${manRoles.map((bes) => `<@&${bes}>`).join(",")} Rolü Verilerek Kayıt Edildi.**`)]})
if(config.selamVer){
...
db.push(`kayıtlar-${message.author.id}`, `\`${config.tagSymbol} ${Name2}` : ""}\` (${womanRoles.map((bes) => `<@&${bes}>`).join(",")} <t:${Math.floor(Date.now() / 1000)}>)`);
...
message.reply({ embeds: [embed.setDescription(`> **<@${member.id}> Kullanıcının ismi** \`${Name2}` : ""}\` **Olarak Değiştirildi**\n> **Ve ${config.womanemoji} ${womanRoles.map((bes) => `<@&${bes}>`).join(",")} Rolü Verilerek Kayıt Edildi.**`)]})
if(config.selamVer){
...
message.channel.send({ embeds: [embed.setDescription(`> **İşlem Başarıyla İptal Edildi!**`)]}).sil(10)
}
collector.stop()
