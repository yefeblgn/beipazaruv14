...
readdir("./src/events", (err, files) => {
  ...
  files.filter((file) => file.endsWith(".js")).forEach((file) => {
    ...
    console.log(`📚 [EFE _ EVENTS] ${prop.conf.name} Yüklendi!`);
  });
});
...
readdir("./src/trackers", (err, files) => {
  if (err) return console.error(err);
  files.filter((file) => file.endsWith(".js")).forEach((file) => {
    let prop = require(`./src/trackers/${file}`);
    if (!prop.conf) return;
    client.on(prop.conf.name, prop);
    console.log(`📩 [EFE _ TRACKERS] ${prop.conf.name} Yüklendi!`);
  });
});
...
client.ceza = async function (id, message, type, reason, durations, süre) {
  ...
  let embed = new EmbedBuilder().setColor("#2f3136").setAuthor({ name: message.member.displayName, iconURL: message.author.avatarURL({ dynamic: true, size: 2048 }) }).setFooter({ text: config.footer ? config.footer : ` Was Here`, iconURL: message.author.avatarURL({ dynamic: true, size: 2048 }) })
  switch (type) {
    case 'BAN':
...
client.kanalbul = function (kanalisim) {
  let kanal = client.guilds.cache.get(config.guildID).channels.cache.find(bes => bes.name === kanalisim)
  if (!kanal) return false;
  return kanal;
}

client.rolbul = function (rolisim) {
  let rol = client.guilds.cache.get(config.guildID).roles.cache.find(bes => bes.name === rolisim)
  if (!rol) return false;
  return rol;
}

client.rolinc = function (rolinc) {
  let rol = client.guilds.cache.get(config.guildID).roles.cache.find(bes => bes.name.toLowerCase().includes(rolinc))
  if (!rol) return false;
  return rol;
}

client.emoji = function (name) {
  let emoji = client.guilds.cache.get(config.guildID).emojis.cache.find(bes => bes.name == name)
