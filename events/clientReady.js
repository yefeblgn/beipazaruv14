...
const tasks = require("../schemas/tasksSchema")
...
let messageTop = [];
let voiceTop = [];
const messageUsersData = await messageUser.find({ guildId: config.guildID }).sort({ topStat: -1 });
const voiceUsersData = await voiceUser.find({ guildId: config.guildID }).sort({ topStat: -1 });
messageUsersData.filter(bes => client.guilds.cache.get(config.guildID).members.cache.get(bes.userId)).splice(0, 10).map(async (x, index) => { messageTop.push({ top: index + 1, avatar: client.guilds.cache.get(config.guildID).members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: client.guilds.cache.get(config.guildID).members.cache.get(x.userId).user.tag, score: `${Number(x.topStat).toLocaleString()}` }) })
voiceUsersData.filter(bes => client.guilds.cache.get(config.guildID).members.cache.get(bes.userId)).splice(0, 10).map(async (x, index) => voiceTop.push({ top: index + 1, avatar: client.guilds.cache.get(config.guildID).members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: client.guilds.cache.get(config.guildID).members.cache.get(x.userId).user.tag, score: `${moment.duration(x.topStat).format("D [Gün], H [sa], m [dk], s [sn]")}` }))
...
if(mr){mr.edit({content:`> **${client.emoji("emote_voice") !== null ? client.emoji("emote_voice"):"🔉"} Ses Sıralaması <t:${Math.floor(Date.now()/1000)}:R> Güncellendi!**`,files:[{attachment:voiceTopCanvas,name:"bes-was-here.png"}]}).catch(err => { })
    ...
    if(bannedTag){setInterval(() => {bannedTag.fives(client,client.guilds.cache.get(config.guildID))}, 5000)}
