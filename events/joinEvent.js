...
member.setNickname(config.kayitsizHesapIsim);
if(fiveImage){
const welcome = await new canvafy.WelcomeLeave()
.setAvatar(member.user.avatarURL({forceStatic:true,extension:"png"}))
.setBackground("image", config.welcomeResimURL)
.setTitle(`${member.user.username}`)
.setDescription("Sunucumuza Hoşgeldin!")
.setBorder(config.welcomeResimRenk)
.setAvatarBorder(config.welcomeResimRenk)
...
if(jailRoles.length > 0)member.roles.set([...jailRoles]);
member.setNickname(config.supheliHesapIsim);
